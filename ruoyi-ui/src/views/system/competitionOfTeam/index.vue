<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="联系人电话区号" prop="contactsAreaCode">
        <el-input
          v-model="queryParams.contactsAreaCode"
          placeholder="请输入联系人电话区号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:competitionOfTeam:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="competitionOfTeamList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="联系人电话区号" align="center" prop="contactsAreaCode" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="联系人电话区号" prop="contactsAreaCode">
          <el-input v-model="form.contactsAreaCode" placeholder="请输入联系人电话区号" />
        </el-form-item>
        <el-form-item label="组内的序号" prop="serialNumber">
          <el-input v-model="form.serialNumber" placeholder="请输入组内的序号" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCompetitionOfTeam(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCompetitionOfTeam(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCompetitionOfTeam(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除赛会中-参赛队伍编号为"' + ids + '"的数据项？').then(function() {
        return delCompetitionOfTeam(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/competitionOfTeam/export', {
        ...this.queryParams
      }, `competitionOfTeam_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
